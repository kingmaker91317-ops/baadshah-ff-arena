...
router.post("/sync", async (req, res) => {
  ...
  if (existing[0]) {
    const updated = await db
      .update(usersTable)
      .set({ email, displayName, photoUrl: photoUrl ?? null })
    ...
    const inserted = await db
      .insert(usersTable)
      .values({ firebaseUid, email, displayName, photoUrl: photoUrl ?? null })
      .returning();
    user = inserted[0];
...
router.patch("/me/ff", requireAuth, async (req, res) => {
  const parsed = FfUpdateBody.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: "Invalid body" }); return; }
  const user = (req as any).currentUser;
  const updated = await db
    .update(usersTable)
    .set({ freeFireUid: parsed.data.freeFireUid, freeFireName: parsed.data.freeFireName })
